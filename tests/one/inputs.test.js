...
test('inputs-basic', (t) => {
  ...
  let g = spacetime({ year: 2015, month: 3, date: 25 })
...
test('missing values', (t) => {
  ...
  t.equal(json.year, 1982, 'got-year')
  t.equal(json.offset, 4, 'got-offset')
  t.equal(json.month, 1, 'jan 1')
  t.equal(json.date, 1, 'jan 1st')

  let now = spacetime.now().json()
  json = spacetime({ month: 2, offset: 4 }).json()
