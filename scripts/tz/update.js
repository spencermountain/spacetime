// CLI orchestration: read zoneinfo, normalize records, report differences, then write.
...
// Directory-only legacy keys may be retained with --allow-unsupported.
// Missing or malformed files remain fatal because their timezone state is unknown.
...
// Library keys are lowercase; zoneinfo paths are case-sensitive on many systems.
// Apply known aliases, then resolve every component using its actual spelling.
export const resolveZone = (directory, name) => {
  ...
  // zdump can silently report UTC for a directory. Reject it before invocation.
  ...
  if (!stats.isFile() || readFileSync(path).subarray(0, 4).toString() !== 'TZif') {
    throw new Error('Not a compiled TZif file: ' + name)
  }
  return path
}

// macOS provides +VERSION; common Linux distributions embed it in tzdata.zi.
// File modification times are not reliable tzdata release identifiers.
...
// Inject the reader so callers can supply fixtures without invoking zdump.
// Build a fresh result and retain diagnostic context without mutating source data.
...
export const main = (args = process.argv.slice(2)) => {
  ...
  const { result, changes, unsupported } = updateZones(data, year, (name) => {
    ...
    // Bound extraction to one UTC year; argument arrays avoid shell interpretation.
    // Interval output includes an initial offset even for zones with no transitions.
  ...
  // Rename within the destination directory so readers never see a partial file.
  // Only remove a temporary file created by this run (exclusive creation may fail).
