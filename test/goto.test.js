...
test('goto-from-algiers (no-dst-places)', t => {
  ...
  t.equal(s.format('nice-full'), 'Tuesday March 14th, 10:48pm', 'unchanged-date');
  //same offset!
  s = s.goto('Africa/Brazzaville');
  t.equal(s.format('nice-full'), 'Tuesday March 14th, 10:48pm', 'same-offset');
  //one to the left...
  ...
  t.equal(s.format('nice-full'), 'Wednesday March 15th, 12:48am', 'two-hours-right-(tomorrow)');
  //three to the right
  s = s.goto('Asia/Dubai');
  t.equal(s.format('nice-full'), 'Wednesday March 15th, 1:48am', 'three-hours-right-(tomorrow)');
  //three and a half to the right...
  s = s.goto('Asia/Kabul');
  t.equal(s.format('nice-full'), 'Wednesday March 15th, 2:18am', 'three-and-a-half-to-the-right-(tomorrow)');
  //back to yesterday..
  s = s.goto('Africa/Dakar');
  t.equal(s.format('nice-full'), 'Tuesday March 14th, 9:48pm', 'back-to-one-hour-left');
  //back to original
  s = s.goto('Africa/Algiers');
  t.equal(s.format('nice-full'), 'Tuesday March 14th, 10:48pm', 'back-to-init-date');
...
test('move-from-not-dst', t => {
  ...
  //in Johannesburg (+2h)
  s = s.goto('Africa/Johannesburg');
  t.equal(s.format('nice-full'), 'Friday March 17th, 11:48pm', 'move-to-joburg');
  t.equal(s.timezone().current.isDST, false, 'joburg-is-never-in-dst');
  t.equal(s.timezone().current.offset, 2, 'joburg-is+2');

  //not-dst London (+0h)
  s = s.goto('Europe/London');
  t.equal(s.format('nice-full'), 'Friday March 17th, 9:48pm', 'init-london');
...
test('move-to-dst', t => {
  ...
  t.equal(s.format('nice-full'), 'Tuesday August 1st, 12:01am', 'init-Moncton');
...
test('move-to-not-dst', t => {
  ...
  t.equal(s.timezone().current.offset, -3, 'uruguay-is-always -3hrs');
  s = s.goto('America/Moncton');
  t.equal(s.format('nice-full'), 'Saturday December 31st, 11:01pm', 'init-Moncton');
