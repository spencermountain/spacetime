...
const isCorrect = function (tz, year) {
  ...
  if (changes.start !== fromDb[0]) {
    console.log('--start--')
    console.log('have', spacetime(changes.start, tz).format('iso'))
    console.log('want', spacetime(fromDb[0], tz).format('iso'))
    console.log('off by: ', (changes.start - fromDb[0]) / 60 / 1000 / 60, 'hrs')
  }
  if (changes.end !== fromDb[1]) {
    console.log('\n--end--')
    console.log('have', spacetime(changes.end, tz).format('iso'))
    console.log('want', spacetime(fromDb[1], tz).format('iso'))
